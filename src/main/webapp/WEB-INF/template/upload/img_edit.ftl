...
$(function(){
	new ToolBar({items:[
		...
		{id:"restoreBut", className:"restore", func:"resetImg('${upLoadFile.width }','${upLoadFile.height }')", text:"还原"},
...
function zoom(param){
	...
	$("#widthRadio").html(Math.round(imgFrame.getPicInfo().width/${upLoadFile.width }*100) /100);
	$("#heightRadio").html(Math.round(imgFrame.getPicInfo().height/${upLoadFile.height }*100) /100);
	rs = emptyRs;
}
...
function save(){
	if(imgFrame.getPicInfo().width == ${upLoadFile.width } && imgFrame.getPicInfo().height == ${upLoadFile.height } && rs.x1 == 0){
		$.getWindow().close();
		return ;
	}
	...
	$.ajax({
		url:'${ctxPath}/file/doEditImg?replaceImage=${replaceImage}',
		type: 'post',
		data: datas,
		dataType: 'json',
		cache: false,
		...
		<div class="edit-form">
			<table cellspacing="0" cellpadding="0" width="100%" id="imgTable">
				...
				<tbody>
					<tr>
						<td>
							<iframe id="imgFrame" name="imgFrame" src="${ctxPath}/file/goSplitImg?fileId=${fileId }&finalWidth=${finalWidth!'' }&finalHeight=${finalHeight!'' }"
								width="100%" height="100%" frameborder="0" scrolling="auto" ></iframe>
						</td>
						<td>
							<div style="width:400px;height: 95px;border:1px solid #879DB1;">
								<span style="color: #183152;font-weight: bold;line-height: 26px;">图片信息：</span><br/>
								<span style="margin-left:5px;">宽度:</span>
								<input type="text" id="userWidth" name="userWidth" value="${upLoadFile.width }" title="截取图片宽度,最大2000px" onkeyup="value=value.replace(/[^\d]/g,'')" value="${finalWidth!''}" style="padding-left:5px;width: 35px;" maxlength="4"/>(px) <br/>
								<span style="margin-left:5px;">高度:</span>
								<input type="text" id="userHeight" name="userHeight" value="${upLoadFile.height }" title="截取图片高度,最大2000px" onkeyup="value=value.replace(/[^\d]/g,'')" value="${finalHeight!''}" style="padding-left:5px;width: 35px;" maxlength="4"/>(px)<br/>
							...
							<div style="width:400px;height: 80px;border:1px solid #879DB1;margin-top: 2px;">
								<span style="color: #183152;font-weight: bold;line-height: 26px;">原图信息：</span><br/>
								<span style="margin-left:5px;">名称：</span> ${upLoadFile.fileName }<br/>
								<span style="margin-left:5px;">像素：</span> ${upLoadFile.width } * ${upLoadFile.height } <br/>
								<span style="margin-left:5px;">大小：</span> ${upLoadFile.fileSize } (KB) <br/>
							</div>
							<div id="emptyDiv">
							</div>
						</td>
