...
<title>添加/编辑资源</title>
<link href="${adminPath}/css/input.css" rel="stylesheet" type="text/css" />
		...
		<form id="inputForm" class="validate" action="" method="post">
			...
			<table class="inputTable">
				...
				<tr>
					...
					<td>
						<input type="text" name="value" class="formText {required: true, remote: 'resource!checkValue.action?oldValue=${(resource.value?url)!}', messages: {remote: '此资源值已存在!'}}" value="${(resource.value)!}" />
						<label class="requireField">*</label>
					</td>
				</tr>
				<tr>
