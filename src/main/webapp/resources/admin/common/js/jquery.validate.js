...
$.extend($.validator, {
	...
	messages: {
		...
		equalTo: "请再次输入相同的值",
		accept: "后缀名不合法",
		maxlength: $.validator.format("长度不超过{0}个字符"),
		minlength: $.validator.format("长度不少于{0}个字符"),
	...
	prototype: {
		...
		form: function() {
			...
			this.showErrors();
			this.focusInvalid(); //增加焦点定位 add by chenld 20120204
			return this.valid();
		},
		...
		defaultMessage: function( element, method) {
			return this.findDefined(
				...
				// title is never undefined, so handle empty string as undefined  wjs
		...
		showLabel: function(element, message) {
			...
			//wjs
			if($(element).parent().find("em").length > 0 && $(element).parent().find("em").attr("wait") == "1"){
				$(element).parent().find("em").show();
				return ;
			}
			if ( label.length ) {
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					if(typeof response == "string")response = $.trim(response); //modified by chenld
		...
		email: function(value, element) {
			...
			return this.optional(element) || /^\s*([A-Za-z0-9_-]+(\.\w+)*@(\w+\.)+\w{2,3})\s*$/.test(value);
		},
...
//						alert('请检查身份证号码长度及其有效性！<br>（15位应全为数字；18位末位可以为数字或X）',ent);
//					}else{
//						alert('请检查身份证号码长度及其有效性！<br>（15位应全为数字；18位末位可以为数字或X）');
//					}
					return false;          
				}
				...
				if(len == 15){
					...
					if(!birthdayValid){
//						if(isValidityParam(ent)){
//							alert('身份证号码中出生日期有误，请检查！',ent);
//						}else{
//							alert('身份证号码中出生日期有误，请检查！');             
//						}
						return false; 
					}
...
//							if(isValidityParam(ent)){
//								alert('18位身份证号码的校验码不正确！应该为：'+valnum,ent);
//							}else{
//								alert('18位身份证号码的校验码不正确！应该为：'+valnum); 
//							}
							return false; 
						} 
						return true; 
		...
		mobile :function(value,element){
			return this.optional(element) || /^0{0,1}13[0-9]{9}$/.test(value);  
		},
...
$.validator.setDefaults({
	submitHandler: function(form) {
		//onsubmit属性置为false, 该submitHandler不起任何作用
	...
	validClass: "success",
	errorElement: "em",
	highlight: function(element, errorClass){}, //校验错误时,标签处理
	unhighlight: function(element, errorClass, validClass){}//纠正错误时,标签处理
	//success: function(label){ //校验成功时
